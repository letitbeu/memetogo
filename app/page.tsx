...
const STORE_KEY = "memetogo:rolling-feed:v1";
const MAX_HISTORY = 120;
const MAX_AGE_MS = 12 * 60 * 60 * 1000;

type FeedResponse = { generatedAt: string; projects: AlphaProject[]; diagnostics?: Array<{chain:string; errors:string[]}>; error?: string };
...
const ago = (epoch: number) => { const s = Math.max(0, Date.now()/1000 - epoch); return s < 60 ? `${Math.floor(s)}秒前` : s < 3600 ? `${Math.floor(s/60)}分钟前` : `${Math.floor(s/3600)}小时前`; };
...
function mergeRolling(previous: AlphaProject[], incoming: AlphaProject[]) {
  ...
  return [...map.values()].filter(row => row.latestSignalEpoch * 1000 >= cutoff).sort((a,b) => b.score-a.score || b.latestSignalEpoch-a.latestSignalEpoch).slice(0, MAX_HISTORY);
}
...
function CandleChart({ candles }: { candles: KlineCandle[] }) {
  ...
  return <div className="chart-wrap"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="24小时5分钟K线">
    ...
    {candles.map((c,i) => { const up=c.close>=c.open; const cx=x(i), yo=y(c.open), yc=y(c.close), yh=y(c.high), yl=y(c.low); return <g key={`${c.time}-${i}`} className={up?"candle-up":"candle-down"}><line x1={cx} x2={cx} y1={yh} y2={yl}/><rect x={cx-candleWidth/2} y={Math.min(yo,yc)} width={candleWidth} height={Math.max(1,Math.abs(yc-yo))}/></g>; })}
...
function ProjectRow({ project, selected, onClick }: {project: AlphaProject; selected: boolean; onClick:()=>void}) {
  return <button className={`project-row ${selected?"selected":""}`} onClick={onClick}>
    ...
    <div className="project-main">
      ...
      <div className="identity-flow"><Badge tone="smart">🧠 聪明钱买入 ×{project.smartBuySignals}</Badge><Badge tone="kol">📣 KOL买入 ×{project.kolBuySignals}</Badge>{project.smartCount>0&&<span>SM持仓 {project.smartCount}</span>}{project.kolCount>0&&<span>KOL持仓 {project.kolCount}</span>}</div>
...
function DetailPanel({ project }: {project: AlphaProject | null}) {
  const [detail,setDetail]=useState<DetailResponse|null>(null); const [loading,setLoading]=useState(false); const [error,setError]=useState("");
  useEffect(()=>{ if(!project){setDetail(null);return;} const ac=new AbortController(); setLoading(true); setError(""); fetch(`/api/token?chain=${encodeURIComponent(project.chain)}&address=${encodeURIComponent(project.address)}`,{cache:"no-store",signal:ac.signal}).then(r=>r.json().then(j=>({ok:r.ok,j}))).then(({ok,j})=>{if(!ok)throw new Error(j.error||"详情加载失败");setDetail(j);}).catch(e=>{if(e.name!=="AbortError")setError(e.message)}).finally(()=>setLoading(false)); return()=>ac.abort(); },[project?.key]);
  if(!project) return <aside className="detail empty-detail"><div><div className="radar-orb">◎</div><h2>选择一个项目</h2><p>查看K线、聪明钱/KOL信号、P0+财富效应、风险和梗文化。</p></div></aside>;
  return <aside className="detail">
    <div className="detail-head"><div><div className="eyebrow">ALPHA DETAIL</div><h2>{project.symbol} <small>{project.name}</small></h2><div className="detail-badges"><Badge tone="smart">🧠 SM ×{project.smartBuySignals}</Badge><Badge tone="kol">📣 KOL ×{project.kolBuySignals}</Badge>{project.legacyP0&&<Badge tone="hot">P0命中</Badge>}{detail?.p0Plus?.confirmed&&<Badge tone="gold">P0+财富效应</Badge>}</div></div><div className="detail-score"><strong>{project.score}</strong><span>{project.grade}</span></div></div>
    ...
    <section><div className="section-title"><h3>Why Now</h3><span>资金优先</span></div><div className="why-grid"><div className="why-card smart"><b>聪明钱</b><strong>{project.smartBuySignals}</strong><span>近期买入信号</span></div><div className="why-card kol"><b>KOL</b><strong>{project.kolBuySignals}</strong><span>近期买入信号</span></div><div className="why-card"><b>市值</b><strong>{money(detail?.info?.marketCap||project.marketCap)}</strong><span>硬门槛 ≥ $1M</span></div><div className="why-card"><b>流动性</b><strong>{money(detail?.info?.liquidity||project.liquidity)}</strong><span>执行容量</span></div></div><ul className="compact-list">{project.thesis.map((x,i)=><li key={i}>{x}</li>)}</ul></section>
    <section><div className="section-title"><h3>P0 / P0+ 验证</h3><span>沿用Newsalert核心阈值</span></div><div className="p0-box"><div><b>P0</b><span>{project.legacyP0?"命中":"未命中"}</span></div><p>{project.legacyP0Reasons.join("；")||"当前仅满足Smart/KOL硬门槛，尚无原P0强度条件。"}</p></div><div className={`p0-box ${detail?.p0Plus?.confirmed?"confirmed":""}`}><div><b>P0+</b><span>{detail?.p0Plus?.confirmed?"确认":"观察"}</span></div><p>{detail?.p0Plus?.reason||"加载后验证Top Trader财富效应"}</p>{detail?.p0Plus?.stories?.map((s:any,i:number)=><div className="wealth-story" key={i}><span>{s.label}</span><b>{money(s.profit)} / {s.roi.toFixed(1)}x</b><code>{short(s.wallet)}</code></div>)}</div></section>
...
export default function Home() {
  const [projects,setProjects]=useState<AlphaProject[]>([]); const [selected,setSelected]=useState<string>(""); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [lastAt,setLastAt]=useState(""); const [chain,setChain]=useState("all"); const [identity,setIdentity]=useState("all");
  const refresh=useCallback(async()=>{ try { const r=await fetch("/api/feed",{cache:"no-store"}); const data:FeedResponse=await r.json(); if(!r.ok)throw new Error(data.error||"Feed加载失败"); setProjects(prev=>{const merged=mergeRolling(prev,data.projects||[]); localStorage.setItem(STORE_KEY,JSON.stringify(merged)); return merged;}); setLastAt(data.generatedAt); setError(""); } catch(e){setError(e instanceof Error?e.message:String(e));} finally{setLoading(false);} },[]);
  useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(STORE_KEY)||"[]");if(Array.isArray(saved))setProjects(mergeRolling([],saved));}catch{} refresh(); const id=setInterval(refresh,15000); return()=>clearInterval(id);},[refresh]);
  const visible=useMemo(()=>projects.filter(p=>(chain==="all"||p.chain===chain)&&(identity==="all"||(identity==="both"?p.smartBuySignals>0&&p.kolBuySignals>0:identity==="smart"?p.smartBuySignals>0:p.kolBuySignals>0))),[projects,chain,identity]);
  const selectedProject=projects.find(p=>p.key===selected)||null;
  const stats=useMemo(()=>({a:projects.filter(p=>p.grade==="A+").length,p0:projects.filter(p=>p.legacyP0).length,both:projects.filter(p=>p.smartBuySignals&&p.kolBuySignals).length}),[projects]);
  return <main><header className="topbar"><div><div className="brand"><span className="brand-mark">M</span><strong>MemeToGo</strong><em>Alpha Radar</em></div><p>Smart Money / KOL First · 先看谁在买，再看价格为什么动</p><p><b>筛选机制：</b>GMGN 实时捕捉 Smart Money / KOL 的真实 BUY，身份资金事件可在低市值阶段先记录；只有项目当前市值 ≥ $1M 才允许上榜，无 Smart Money/KOL 买入直接淘汰。通过硬门槛后，再按身份资金强度 → P0 强度 → 5分钟成交/流动性等市场微结构 → 风险惩罚计算 Alpha Score 并排序。</p></div><div className="live"><span className="live-dot"/><div><b>LIVE</b><small>{lastAt?new Date(lastAt).toLocaleTimeString("zh-CN",{hour12:false}):"连接中"}</small></div></div></header>
