...
const CULTURE_CACHE_MS = 30 * 60 * 1000;
const cultureCache = new Map<string, { at: number; value: CultureResearch }>();
const cultureInflight = new Map<string, Promise<CultureResearch>>();
...
function cacheKey(meta: Record<string, unknown>) {
  return [meta.symbol, meta.name, meta.twitter, meta.website, meta.description].map(v => String(v || "")).join("|").slice(0, 1600);
}

async function generateCulture(meta: Record<string, unknown>): Promise<CultureResearch> {
...
export async function researchCulture(meta: Record<string, unknown>): Promise<CultureResearch> {
  const id = cacheKey(meta);
  const now = Date.now();
  const cached = cultureCache.get(id);
  if (cached && now - cached.at < CULTURE_CACHE_MS) return cached.value;

  let pending = cultureInflight.get(id);
  if (!pending) {
    pending = generateCulture(meta)
      .then(value => {
        cultureCache.set(id, { at: Date.now(), value });
        while (cultureCache.size > 64) {
          const oldest = cultureCache.keys().next().value as string | undefined;
          if (!oldest) break;
          cultureCache.delete(oldest);
        }
        return value;
      })
      .finally(() => cultureInflight.delete(id));
    cultureInflight.set(id, pending);
  }
  return pending;
}
